...
setGeneric("metagene_plot",
           function(object,
                    ...
                    facet_wrap_params = list(),...) standardGeneric("metagene_plot"))
...
#' method for metagene_plot
#'
#' @rdname metagene_plot
#' @exportMethod metagene_plot
setMethod("metagene_plot",
          ...
          function(object,
              ...
              if(mode == "codon"){
                if(frame == TRUE){
                  ...
                  cb_df <- rbind(tmp_codon_positive,tmp_codon_negtive) %>%
                    dplyr::group_by(sample,group,rep,codon_pos,frame) %>%
                  ...
                  cb_df <- rbind(tmp_codon_positive,tmp_codon_negtive) %>%
                    dplyr::group_by(sample,group,rep,codon_pos) %>%
                    dplyr::summarise(avergae_exp = sum(avergae_exp)) %>%
                    dplyr::mutate(pos = codon_pos)
                }
              }else{
            ...
            if(merge_rep == TRUE){
              ...
              if(collapse == TRUE){
                line_layer <- do.call(geom_line,modifyList(list(mapping = aes(x = pos,y = mean_exp,color = rep),
                                                                linewidth = linewidth),
                                                           geom_line_params))
