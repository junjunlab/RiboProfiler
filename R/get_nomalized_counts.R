...
#'        Default is 5.
#' The nomalization methods for ribosome density. "average" is calculated by
...
setGeneric("get_nomalized_counts",
           function(object,
                    ...
                    exclude_downstreamCodon = 5,...) standardGeneric("get_nomalized_counts"))
...
setMethod("get_nomalized_counts",
          ...
          function(object,
            ...
            purrr::map_df(seq_along(sp),function(x){
              ...
              # normalization methods rpm
              total_counts <- sum(tmp$counts)
              tmp$rpm <- (tmp$counts/total_counts)*10^6

              # normalization methods average
              total_exp <- tmp %>%
              ...
              qc_df_new <- tmp %>%
                ...
                dplyr::mutate(average = counts/average_exp) %>%
